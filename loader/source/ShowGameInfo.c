...
static void DrawGameInfoScreen(const gameinfo *gi, const MD5VerifyState_t *md5)
{
	...
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*6,  "名 称 :         %s", gi->Name);
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*7,  "游戏ID:         %.6s", gi->ID);

	static const char *const BI2regions[4] = {"Japan", "USA", "PAL", "South Korea"};
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*8,  "区 域 :         %s",
		BI2regions[(gi->Flags & GIFLAG_REGION_MASK) >> 3]);

	const u8 discNumber = ((gi->Flags & GIFLAG_DISCNUMBER_MASK) >> 5);
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*9,  "版 本 :         %02u", gi->Revision);
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*10, "碟 #  :         %u", discNumber+1);
	...
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*11, "格 式 :");
