...
static bool UpdateGameSelectMenu(MenuCtx *ctx)
{
	...
	if (ctx->redraw)
	{
		...
		PrintFormat(DEFAULT_SIZE, DiscFormatColors[5], MENU_POS_X+(36*10), MENU_POS_Y + 20*3, "超大");
...
static bool UpdateSettingsMenu(MenuCtx *ctx)
{
	...
	if (ctx->redraw)
	{
		...
		// BBA Emulation
		PrintFormat(MENU_SIZE, BLACK, MENU_POS_X + 320, SettingY(ListLoopIndex),
			    "%-18s:%-4s", "BBA Emulation", (ncfg->Config & (NIN_CFG_BBA_EMU)) ? "On" : "关闭");
		ListLoopIndex++;
	...
	while(1)
	{
		...
		if (ctx.redraw)
		{
			...
			if (ctx.menuMode == 0)
			{
				...
				// If the selected game bootable, enable "选 择".
...
void PrintInfo(void)
{
	...
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*0, "Nintendont Loader v%u.%u (%s)",
		    NIN_VERSION>>16, NIN_VERSION&0xFFFF, consoleType);
#endif
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*1, "编译日期: " __DATE__ " " __TIME__);
...
void PrintLoadKernelError(LoadKernelError_t iosErr, int err)
{
	...
	switch (iosErr)
	{
		...
		case LKERR_IOS_Open_shared1_content_map:
			...
			PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, MENU_POS_Y + 20*10, "请确保 Nintendont 目录中的 meta.xml");
			PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, MENU_POS_Y + 20*11, "存在且包含");
			PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, MENU_POS_Y + 20*12, "<ahb_access/> 标签.");
			break;
